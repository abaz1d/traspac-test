...
function hitungKemunculanKata(kata: string, artikel: string) {
  const regex = new RegExp(kata, "gi"); // cek semua dengan case-insensitive, hapus "i", untuk case-sensitive
  const jumlahKemunculan = (artikel.match(regex) || []).length; // menjumlah total/ panjang kata yang didapat
  return jumlahKemunculan;
}

function gantiKata(kataLama: string, kataBaru: string, artikel: string) {
  const regex = new RegExp(kataLama, "gi"); // cek semua dengan case-insensitive, hapus "i", untuk case-sensitive
  const teksBaru = artikel.replace(regex, kataBaru); // replace kata lama(kiri) diganti kata baru (kanan)
  return teksBaru;
}

function urutkanKata(artikel: string) {
  const kataArray = artikel
    .split(/\s+/)
    .filter((kata) => kata.trim().length > 0); // memisahkan artikel menjadi []kata kemudian menghapus spasi
  kataArray.sort(); // mengurutkan kata
  return kataArray.join(" "); // beri spasi
}

const kataCari = "teknologi";
const kataBaru = "ilmu pengetahuan";

console.log("+------------------------------------------------------------+");
console.log("|                  Analisis Artikel                          |");
console.log("+------------------------------------------------------------+");
const jumlahKemunculan = hitungKemunculanKata(kataCari, artikel);
console.log("| Jumlah kemunculan kata dalam artikel:                      |");
console.log(
  `| Kata "${kataCari}" muncul sebanyak ${jumlahKemunculan} kali             |`
);
console.log("+------------------------------------------------------------+");
const artikelDenganPenggantian = gantiKata("adalah", "ialah", artikel);
console.log("| Artikel setelah penggantian kata:                         |");
console.log("|", artikelDenganPenggantian);
console.log("+------------------------------------------------------------+");
const kataYangDiurutkan = urutkanKata(artikel);
